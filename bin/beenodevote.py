...
def run_beenode_cli_command(cmd):
    return run_command("%s %s" % (beenode_cli_path or 'beenode-cli', cmd))
...
def submit_votes(win, ballot, s):
    ...
    if votes_to_send.keys():
        ...
        for mn in shuffled_mn_keys:
            ...
            if background_send:
                deferred_votes = tempfile.NamedTemporaryFile(prefix=('sending_beenodevote_votes-%s=' % batch_timestamp),delete=False)
            ...
            for vote in shuffled_vote_keys:
                ...
                signature = beenodeutil.sign_vote(netvote, mnprivkey)
                command = ('%s' % beenode_cli_path is not None and beenode_cli_path or 'beenode-cli') + ' voteraw ' + str(node['txid']) + ' ' + str(node['txout']) + ' ' + str(
        ...
        if background_send:
            voter_parent = tempfile.NamedTemporaryFile(prefix=('sending_beenode_votes-%s-' % batch_timestamp), delete=False)
...
def main(screen):
    ...
    if beenode_cli_path is None:
        # test beenode-cli in path -- TODO make robust
        try:
            run_command('beenode-cli getinfo')
        except subprocess.CalledProcessError:
            quit(
                "--> cannot find beenode-cli in $PATH\n" +
                "    do: export PATH=/path/to/beenode-cli-folder:$PATH\n" +
                "    and try again\n")

    loadwin = curses.newwin(40, 40, 1, 2)

    loadwin.addstr(1, 2, 'beenodevote version: ' + version, C_CYAN)

    mncount = int(run_beenode_cli_command('masternode count enabled'))
    block_height = int(run_beenode_cli_command('getblockcount'))
    ...
    # get ballot
    ballots = json.loads(run_beenode_cli_command('gobject list all'))
    ballot = {}
    ...
    for entry in ballots:
        ...
        ballots[entry][u'votes'] = json.loads(run_beenode_cli_command('gobject getvotes %s' % entry))
    ...
    masternodes = {}
    with open(os.path.join(beenode_conf_dir, 'masternode.conf'), 'r') as f:
        lines = list(
            line
            for line in
            (l.strip() for l in f)
        ...
        for line in lines:
            ...
            masternodes[conf[3] + '-' + conf[4]] = {
                ...
                "txout": conf[4]}
    if not masternodes:
        # fallback to beenode.conf entries if no masternode.conf entries
        with open(os.path.join(beenode_conf_dir, 'beenode.conf'), 'r') as f:
            lines = list(
                line
                for line in
                (l.strip() for l in f)
            ...
            if all(k in conf for k in ('masternode', 'externalip', 'masternodeprivkey')):
                # get funding tx from beenodeninja
                import urllib2
                mninfo = urllib2.urlopen(
                    "https://beenodeninja.pl/api/masternodes?ips=[\"" +
                ...
                try:
                    ...
                    quit('cannot retrieve masternode info from beenodeninja')
                ...
                quit('cannot find masternode information in beenode.conf')
    ...
    votewin.addstr(1, 2, 'beenodevote version: ' + version, C_CYAN)
    votewin.addstr(
        2,
        2,
