...
# beenodeman_functions.sh - common functions and variables
...
CACHE_DIR=/tmp/beenodeman_cache
mkdir -p $CACHE_DIR
chmod 700 $CACHE_DIR
...
# attempt to locate beenode-cli executable.
# search current dir, ~/.beenode, `which beenode-cli` ($PATH), finally recursive
_find_beenode_directory() {
    ...
    if [ ! -z $(which beenode-cli 2>/dev/null) ] ; then
        INSTALL_DIR=$(readlink -f `which beenode-cli`)
        INSTALL_DIR=${INSTALL_DIR%%/beenode-cli*};
        ...
        if [[ $INSTALL_DIR =~ \/usr.* ]]; then
            ...
            # if not run as root
            if [ $EUID -ne 0 ] ; then
                die "\n${messages["exec_found_in_system_dir"]} $INSTALL_DIR${messages["run_beenodeman_as_root"]} ${messages["exiting"]}"
            fi
        fi

    # beenode-cli not in PATH

        # check current directory
    elif [ -e ./beenode-cli ] ; then
        INSTALL_DIR='.' ;

        # check ~/.beenode directory
    elif [ -e $HOME/.beenode/beenode-cli ] ; then
        INSTALL_DIR="$HOME/.beenode" ;

    elif [ -e $HOME/.beenodecore/beenode-cli ] ; then
        INSTALL_DIR="$HOME/.beenodecore" ;

        # TODO try to find beenode-cli with find
#    else
#        CANDIDATES=`find $HOME -name beenode-cli`
    fi

    if [ ! -z "$INSTALL_DIR" ]; then
        INSTALL_DIR=$(readlink -f $INSTALL_DIR) 2>/dev/null
        if [ ! -e $INSTALL_DIR ]; then
            echo -e "${C_RED}${messages["beenodecli_not_found_in_cwd"]}, ~/.beenodecore, or \$PATH. -- ${messages["exiting"]}$C_NORM"
            exit 1
        fi
    else
        echo -e "${C_RED}${messages["beenodecli_not_found_in_cwd"]}, ~/.beenodecore, or \$PATH. -- ${messages["exiting"]}$C_NORM"
    ...
    if [ ! -e $INSTALL_DIR/beenoded ]; then
        echo -e "${C_RED}${messages["beenoded_not_found"]} $INSTALL_DIR -- ${messages["exiting"]}$C_NORM"
        exit 1
    fi
...
_get_platform_info() {
    ...
    case "$PLATFORM" in
        ...
        *)
            ...
            err "${messages["err_beenodeman_supports"]}"
            die "${messages["exiting"]}"
            ;;
    esac
}
...
_get_versions() {
    ...
    TARDIR="beenodecore-${LATEST_VERSION::-2}"
...
update_beenoded(){
    ...
    if [ $LATEST_VERSION != $CURRENT_VERSION ] || [ ! -z "$REINSTALL" ] ; then
        ...
        if [ ! -z "$REINSTALL" ];then
            echo -e ""
            echo -e "$C_GREEN*** ${messages["beenode_version"]} $CURRENT_VERSION is up-to-date. ***$C_NORM"
            echo -e ""
            echo -en
            ...
            echo -e ""
            echo -e "$C_RED*** ${messages["newer_beenode_available"]} ***$C_NORM"
            echo -e ""
        ...
        wget --no-check-certificate -q -r https://github.com/bee-group/beenode/releases/download/v$LATEST_VERSION/SHA256SUMS.asc -O ${DOWNLOAD_FILE}.DIGESTS.txt
        ...
        rm -rf \
            ...
            sporks.dat \
            beenoded \
            beenoded-$CURRENT_VERSION \
            beenode-qt \
            beenode-qt-$CURRENT_VERSION \
            beenode-cli \
            beenode-cli-$CURRENT_VERSION \
            beenodecore-${CURRENT_VERSION}*.gz*
        ok "${messages["done"]}"

        # place it ---------------------------------------------------------------

        mv $TARDIR/bin/beenoded beenoded-$LATEST_VERSION
        mv $TARDIR/bin/beenode-cli beenode-cli-$LATEST_VERSION
        if [ $PLATFORM != 'armv7l' ];then
            mv $TARDIR/bin/beenode-qt beenode-qt-$LATEST_VERSION
        fi
        ln -s beenoded-$LATEST_VERSION beenoded
        ln -s beenode-cli-$LATEST_VERSION beenode-cli
        if [ $PLATFORM != 'armv7l' ];then
            ln -s beenode-qt-$LATEST_VERSION beenode-qt
        fi

        # permission it ----------------------------------------------------------

        if [ ! -z "$SUDO_USER" ]; then
            chown -h $SUDO_USER:$SUDO_USER {$DOWNLOAD_FILE,${DOWNLOAD_FILE}.DIGESTS.txt,beenode-cli,beenoded,beenode-qt,beenode*$LATEST_VERSION}
        fi

        # purge it ---------------------------------------------------------------

        rm -rf beenode-0.12.0
        rm -rf beenodecore-0.12.1*
        rm -rf beenodecore-0.12.2*
        rm -rf beenodecore-0.12.3*
        rm -rf $TARDIR

        # punch it ---------------------------------------------------------------

        pending " --> ${messages["launching"]} beenoded... "
        touch $INSTALL_DIR/beenoded.pid
        ...
        MN_CONF_ENABLED=$( egrep -s '^[^#]*\s*masternode\s*=\s*1' $INSTALL_DIR/beenode.conf | wc -l 2>/dev/null)
        if [ $MN_CONF_ENABLED -gt 0 ] ; then
        ...
        if [ $LATEST_VERSION == $CURRENT_VERSION ]; then
            ...
            echo -e ""
            ls -l --color {$DOWNLOAD_FILE,${DOWNLOAD_FILE}.DIGESTS.txt,beenode-cli,beenoded,beenode-qt,beenode*$LATEST_VERSION}
            echo -e ""

            quit
        else
            echo -e "${C_RED}${messages["beenode_version"]} $CURRENT_VERSION ${messages["is_not_uptodate"]} ($LATEST_VERSION) ${messages["exiting"]}$C_NORM"
        fi

    else
        echo -e ""
        echo -e "${C_GREEN}${messages["beenode_version"]} $CURRENT_VERSION ${messages["is_uptodate"]} ${messages["exiting"]}$C_NORM"
    fi
...
#    fi
    ...
    if [ ! -e $INSTALL_DIR/beenode.conf ] ; then
        pending " --> ${messages["creating"]} beenode.conf... "
...
#        fi
    ...
    $wget_cmd -O - https://github.com/bee-group/beenode/releases/download/v$LATEST_VERSION/SHA256SUMS.asc | pv -trep -w80 -N checksums > ${DOWNLOAD_FILE}.DIGESTS.txt
    echo -ne "$C_NORM"
    clear_n_lines 2
    tput rc
    clear_n_lines 3
...
#    ok "${messages["done"]}"
    ...
    BOOSTRAP_LINKS='https://raw.githubusercontent.com/UdjinM6/beenode-bootstrap/master/links-mainnet.md'
    ...
    MAINNET_BOOTSTRAP_FILE_1_SIZE_M=$(( $(echo $MAINNET_BOOTSTRAP_FILE_1_SIZE | sed -e 's/[^0-9]//g')  ))
    ...
    if [ ! -s $MAINNET_BOOTSTRAP_FILE ]; then
        ...
        tput sc
        echo -e "$C_CYAN"
        BOOTSTRAP_SIZE_M=$(( $(unzip -l ${MAINNET_BOOTSTRAP_FILE##*/} | grep -v zip | grep bootstrap.dat | awk '{print $1}')  ))
    ...
    sed -i.bak -e '/beenodeman_env/d' ~/.bash_aliases
    echo "export PATH=$INSTALL_DIR:\$PATH ; # beenodeman_env" >> ~/.bash_aliases
    ok "${messages["done"]}"
    ...
    if [ $LATEST_VERSION == $CURRENT_VERSION ]; then
        echo -e ""
        echo -e "${C_GREEN}beenode ${LATEST_VERSION} ${messages["successfully_installed"]}$C_NORM"
        ...
        if [ ! -z "$SUDO_USER" ]; then
            ...
            echo -e ""
            ls -l --color $LINK_TO_SYSTEM_DIR/{beenoded,beenode-cli}
            echo -e ""
        fi
...
get_beenoded_status(){
    ...
    WEB_BLOCK_COUNT_DQA=`$curl_cmd https://explorer.beenode.org/chain/Dash/q/getblockcount`;
    ...
    MN_PROTX_QUEUE=`echo "$MN_PROTX_RAW" | egrep '(proTxHash|lastPaidHeight|PoSeRevivedHeight|registeredHeight)' | sed -e 's/[":,{}]//g' -e 's/^ \+//' -e 's/ \+$//' -e '/^$/d' -e '/^[^ ]\+$/d' | sed -e 'N;s/\n/ /' | sed -e 'N;s/\n/ /' | awk ' \
{
    ...
    if [ $MN_CONF_ENABLED -gt 0 ] ; then
        WEB_NINJA_API=$($curl_cmd "https://www.beenodeninja.pl/api/masternodes?ips=\[\"${MASTERNODE_BIND_IP}:9999\"\]&portcheck=1&balance=1")
        if [ -z "$WEB_NINJA_API" ]; then
            sleep 2
            # downgrade connection to support distros with stale nss libraries
            WEB_NINJA_API=$($curl_cmd --ciphers rsa_3des_sha "https://www.beenodeninja.pl/api/masternodes?ips=\[\"${MASTERNODE_BIND_IP}:9999\"\]&portcheck=1&balance=1")
        fi
...
show_message_configure() {
    ...
    echo
         pending "    $HOME/.beenodecore/beenode.conf" ; echo
    echo
    echo -e "$C_GREEN install sentinel$C_NORM"
    echo
    echo -e "    ${C_YELLOW}beenodeman install sentinel$C_NORM"
    echo
    echo -e "$C_GREEN ${messages["then_run"]}$C_NORM"
    echo
    echo -e "    ${C_YELLOW}beenodeman restart now$C_NORM"
    echo
}
...
install_sentinel() {
    ...
    git clone -q https://github.com/bee-group/sentinel.git
